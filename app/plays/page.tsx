...
export default function Plays() {
  ...
  const genres = ['الكل', 'أكشن', 'كوميديا', 'دراما','رومانسي'];

  
  const filteredMovies = plays.filter(play => 
    selectedGenre === 'الكل' || play.genre === selectedGenre
  );
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="pt-24 px-4 md:px-8 lg:px-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6 pb-16">
          {sortedMovies.map((play) => (
            <MovieCard key={`rec-${play.id}`} movie={play} type="plays" />
          ))}
        </div>
      </div>
