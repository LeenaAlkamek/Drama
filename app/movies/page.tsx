...
export default function Movies() {
  ...
  const genres = ['الكل', 'أكشن', 'كوميديا', 'دراما', 'خيال علمي', 'رومانسي', 'إثارة', 'رعب', 'مغامرة', 'وثائقي'];

  
  const filteredMovies = movies.filter(movie => 
    selectedGenre === 'الكل' || movie.genre === selectedGenre
  );
      ...
      <div className="pt-24 px-4 md:px-8 lg:px-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6 pb-16">
          {sortedMovies.map((movie) => (
            <MovieCard key={`rec-${movie.id}`} movie={movie} type="movies" />
          ))}
        </div>
      </div>
