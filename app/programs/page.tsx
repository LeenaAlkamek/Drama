...
export default function Programs() {
  ...
  const genres = ['الكل', 'كوميديا', 'خيال علمي', 'ديني', 'اجتماعي', 'وثائقي'];

  
  const filteredMovies = programs.filter(program => 
    selectedGenre === 'الكل' || program.genre === selectedGenre
  );
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="pt-24 px-4 md:px-8 lg:px-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6 pb-16">
          {sortedMovies.map((program) => (
            <MovieCard key={`rec-${program.id}`} movie={program} type="programs" />
          ))}
        </div>
      </div>
